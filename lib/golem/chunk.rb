module Golem
  ...
  class Chunk
    ...
    def initialize(x, z, data)
      @x, @z = x, z
      data = Zlib::Inflate.inflate(data)
      *block_types = data[0...PER_CHUNK].unpack("c#{PER_CHUNK}")
    ...
    def find(type, absolute=true)
      ...
      blocks.each.with_index do |b, i|
        if b == type
          x, y, z = [i / (16*128), i % 128, i % (16*128) / 128]
          if absolute
            found << [x + self.x, y, z + self.z]
          else
            found << [x, y, z]
          end
        end
